...
import com.Mihin.BookStore.testAuthorDtoA
import com.Mihin.BookStore.testAuthorEntityA
import com.Mihin.BookStore.testAuthorUpdateRequestDtoA
import com.Mihin.BookStore.testAuthorUpdateRequestDtoA
...
class AuthorControllerTest @Autowired constructor (
    ...
    @Test
    fun `test that create Author saves the Auhtor` () {
    ...
    @Test
    fun ` test that creates Author returns HTTP 400 when IllegalArgumentException `() {
        every{
            authorService.create(any())
    ...
    @Test
    fun `test that list returns authors and HTTP 200 when no authors in a database` () {
        every{
            authorService.list()
        } answers{
    ...
    @Test
    fun `test that get returns HTTP 200 and author found `(){
        every {
            authorService.get(any())
        } answers {
    ...
    @Test
    fun `test that full update Author returns HTTP 400 on IllegalArgumentException` (){
        every{
            authorService.fullUpdate(any(), any())
