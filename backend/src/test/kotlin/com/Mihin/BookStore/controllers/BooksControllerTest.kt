...
class BooksControllerTest @Autowired constructor (
    ...
    fun `test that createFullUpdateBook returns HTTp 201 when book is created `(){
        asserThatUserCreatedUpdated(true) {
            isCreated()
        }
    }
    ...
    fun `test that createFullUpdateBook returns HTTP 200 when book is updated `(){
        asserThatUserCreatedUpdated(false) {
            isOk()
        }
    }


    private fun asserThatUserCreatedUpdated(isCreated: Boolean, statusCodeAssertion: StatusResultMatchersDsl.() -> Unit){
    ...
    @Test
    fun `test that createFullUpdateBook returns HTTP 500 when author in database does not have an ID`(){
        val isbn = "978-089-230342-0777"
    ...
    @Test
    fun `test that createFullUpdate return HTTP 400 when author does not exist` () {
        val isbn = "978-089-230342-0777"
    ...
    @Test
    fun `test that bookPartialUpdate returns a HTTP 400 on a IllegalStateException `() {
